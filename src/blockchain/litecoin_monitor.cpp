...
// Litecoin RPC Client (same as Bitcoin)
class LitecoinRPCClient {
public:
    LitecoinRPCClient(const std::string& url, const std::string& user, const std::string& password)
...
// LitecoinMonitor implementation
struct LitecoinMonitor::Impl {
    std::string rpc_url;
    std::string rpc_user;
    ...
    std::unique_ptr<LitecoinRPCClient> rpc_client;
    ...
    Impl(const std::string& url, const std::string& user, const std::string& password, bool testnet_flag)
        : rpc_url(url), rpc_user(user), rpc_password(password), testnet(testnet_flag),
          blockchain_type(testnet_flag ? BlockchainType::TESTNET_LTC : BlockchainType::LITECOIN),
          is_active(false) {
        rpc_client = std::make_unique<LitecoinRPCClient>(url, user, password);
    }
    ...
    void MonitorLoop() {
        while (is_active) {
            ...
            try {
                ...
                LogF(LogLevel::ERROR, "Litecoin monitor error: %s", e.what());
            }

            // Sleep for 2.5 seconds between scans (Litecoin has faster blocks)
            std::this_thread::sleep_for(std::chrono::milliseconds(2500));
        }
    }

    void ScanRecentBlocks(uint64_t current_height) {
        // Scan last 24 blocks for HTLC transactions (6 minutes at 2.5min/block)
        uint64_t start_height = current_height > 24 ? current_height - 24 : 0;
...
LitecoinMonitor::LitecoinMonitor(const std::string& rpc_url,
...
LitecoinMonitor::~LitecoinMonitor() {
    Stop();
}

Result<void> LitecoinMonitor::Start() {
    if (impl_->is_active) {
    ...
    LogF(LogLevel::INFO, "Litecoin monitor started");
    return Result<void>::Ok();
}

Result<void> LitecoinMonitor::Stop() {
    if (!impl_->is_active) {
        return Result<void>::Ok();
    }
    ...
    LogF(LogLevel::INFO, "Litecoin monitor stopped");
    return Result<void>::Ok();
}

bool LitecoinMonitor::IsActive() const {
    return impl_->is_active;
}

BlockchainType LitecoinMonitor::GetBlockchainType() const {
    return impl_->blockchain_type;
}

Result<uint64_t> LitecoinMonitor::GetCurrentBlockHeight() {
    return impl_->GetCurrentBlockHeight();
}

Result<uint256> LitecoinMonitor::GetCurrentBlockHash() {
...
Result<void> LitecoinMonitor::WatchForHTLC(
    const std::vector<uint8_t>& payment_hash,
    ...
    LogF(LogLevel::INFO, "Now watching for Litecoin HTLC with payment hash: %s",
         BytesToHex(payment_hash).c_str());

    return Result<void>::Ok();
}

Result<void> LitecoinMonitor::StopWatchingHTLC(const std::vector<uint8_t>& payment_hash) {
    std::lock_guard<std::mutex> lock(impl_->watched_htlcs_mutex);
...
Result<HTLCTransaction> LitecoinMonitor::GetHTLCTransaction(
    ...
    if (tx.isMember("confirmations")) {
        ...
        htlc_tx.status = htlc_tx.confirmations >= 24 ? TxStatus::CONFIRMED : TxStatus::CONFIRMING;
    } else {
...
Result<uint32_t> LitecoinMonitor::GetConfirmations(const uint256& tx_hash) {
    std::string txid = BytesToHex(std::vector<uint8_t>(tx_hash.begin(), tx_hash.end()));
...
Result<std::vector<uint8_t>> LitecoinMonitor::WatchForPreimage(
...
Result<bool> LitecoinMonitor::IsHTLCSpent(
    const uint256& htlc_tx_hash,
    uint32_t htlc_output_index) {
...
Result<uint256> LitecoinMonitor::BroadcastTransaction(const std::string& raw_tx_hex) {
    Json::Value params(Json::arrayValue);
...
void LitecoinMonitor::OnHTLCDetected(HTLCDetectedCallback callback) {
    std::lock_guard<std::mutex> lock(impl_->callbacks_mutex);
    impl_->htlc_detected_callback = callback;
}

void LitecoinMonitor::OnHTLCConfirmed(HTLCConfirmedCallback callback) {
    std::lock_guard<std::mutex> lock(impl_->callbacks_mutex);
    impl_->htlc_confirmed_callback = callback;
}

void LitecoinMonitor::OnHTLCClaimed(HTLCClaimedCallback callback) {
    std::lock_guard<std::mutex> lock(impl_->callbacks_mutex);
    impl_->htlc_claimed_callback = callback;
}

void LitecoinMonitor::OnHTLCRefunded(HTLCRefundedCallback callback) {
    std::lock_guard<std::mutex> lock(impl_->callbacks_mutex);
