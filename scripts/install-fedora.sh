...
# INTcoin Installation Script for Fedora/RHEL/CentOS
# Supports: Fedora 35+, RHEL 8+, Rocky Linux 8+, AlmaLinux 8+
...
detect_os() {
    if [ -f /etc/os-release ]; then
        ...
        print_error "Cannot detect OS. This script supports Fedora/RHEL/CentOS only."
        exit 1
    fi
    ...
    case "$OS" in
        fedora|rhel|centos|rocky|almalinux)
            ...
            ;;
        *)
            print_error "This script is for Fedora/RHEL/CentOS. Detected: $OS"
            print_info "Use install-debian.sh for Debian/Ubuntu or install-arch.sh for Arch Linux"
            exit 1
            ;;
    esac
}
...
install_dependencies() {
    ...
    if [[ "$OS" == "rhel" || "$OS" == "rocky" || "$OS" == "almalinux" ]]; then
        if [[ "$VER" == "8"* ]]; then
...
main() {
    ...
    print_info "Supported: Fedora 35+, RHEL 8+, Rocky Linux 8+, AlmaLinux 8+"
    echo ""
