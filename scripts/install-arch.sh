...
# INTcoin Installation Script for Arch Linux
# Supports: Arch Linux, Manjaro, EndeavourOS
...
detect_os() {
    if [ -f /etc/os-release ]; then
        ...
        print_error "Cannot detect OS. This script supports Arch Linux only."
        exit 1
    fi
    ...
    case "$OS" in
        arch|manjaro|endeavouros)
            ...
            ;;
        *)
            print_error "This script is for Arch Linux. Detected: $OS"
            print_info "Use install-debian.sh for Debian/Ubuntu or install-fedora.sh for Fedora/RHEL"
            exit 1
            ;;
    esac
}
...
install_dependencies() {
    ...
    if ! pkg-config --exists liboqs; then
        ...
        # Try from AUR first (if yay/paru is available)
        if command -v yay &> /dev/null; then
            yay -S --noconfirm liboqs
            print_success "liboqs installed from AUR"
        elif command -v paru &> /dev/null; then
            paru -S --noconfirm liboqs
            print_success "liboqs installed from AUR"
        else
            # Build from source
            ...
            print_success "liboqs installed from source"
        fi
    else
...
main() {
    ...
    print_info "Supported: Arch Linux, Manjaro, EndeavourOS"
    echo ""
